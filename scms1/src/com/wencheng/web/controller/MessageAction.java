...
public class MessageAction extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(ms.create(request)){
			response.sendRedirect(request.getContextPath()+"/student/message");
			return;
		}else{
			response.sendRedirect(request.getContextPath()+"/student/message?errormessage="+URLEncoder.encode("创建失败，请重试","utf-8"));
			return;
		}
	}
