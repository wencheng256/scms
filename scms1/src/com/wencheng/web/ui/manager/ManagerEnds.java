...
public class ManagerEnds extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(m.getType() == 0){
			list = dao.listEnd(Integer.parseInt(grade),start,rows);
		}else{
			list = dao.listSchoolEnd(Integer.parseInt(grade),m.getSchool().getId(), start,rows);
		}
		...
		//dispatcher
		//dispatcher
		request.getRequestDispatcher("/WEB-INF/views/upper/manager/ends.jsp").forward(request, response);
	}
