...
public class ManagerMiddles extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public ManagerMiddles() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(m.getType() == 0){
			list = dao.listMiddle(Integer.parseInt(grade),start,rows);
		}else{
			list = dao.listSchoolMiddle(Integer.parseInt(grade),m.getSchool().getId(), start,rows);
		}
		...
		//dispatcher
		//dispatcher
		request.getRequestDispatcher("/WEB-INF/views/upper/manager/middles.jsp").forward(request, response);
	}
